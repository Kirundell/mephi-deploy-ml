...
Разные типы моделей дают более интересный A/B-тест:
...
| Метрика | Описание | Importance |
|---------|----------|------------|
| **F1-score** | Основная метрика качества | Primary |
| **Recall** | Полнота обнаружения дефолтов | High |
| **Precision** | Точность прогнозов | Medium |
| **ROC-AUC** | Общая ранжирующая способность | Medium |
...
Typically, loss from FN >> loss from FP, поэтому Recall важнее Precision.
...
# Если есть ground truth (из delayed feedback):
# merged = df.merge(labels, on='user_id')
# f1_v1 = f1_score(merged[merged['model_version']=='v1']['true_label'],
#                  merged[merged['model_version']=='v1']['prediction'])
```
...
1. **Не останавливайте тест рано** — подглядывание данных искажает результаты
2. **Фиксируйте план заранее** — не меняйте критерии в процессе
3. **Сегментный анализ** — проверьте, что результаты согласованы по сегментам
4. **Документируйте всё** — для воспроизводимости

## Риски

- **Cannibalization:** пользователи из разных групп взаимодействуют
- **External factors:** изменения в экономике во время теста
- **Data contamination:** утечка информации между группами
- **Seasonality:** особенности периода (праздники, конец месяца)

## Дальнейшее улучшение

1. **Multi-armed bandit:** динамическое перераспределение трафика
2. **Sequential testing:** более ранний останов при значимых различиях
3. **CUPED:** уменьшение дисперсии с использованием ковариат
